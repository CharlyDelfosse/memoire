...
def graphgame_viz(bdd, gg):
    ...
    for curr_node in all_nodes:
        if bdd.let(curr_node, gg.phi_0) == bdd.true:
            curr_prios = get_prios(bdd, gg, curr_node)
            ...
            nx_graph.nodes[node_id]["node_info"] = str(curr_prios) + " , " + str(
                list(map(boolean_to_const, node_infos(curr_node))))
...
def get_prios(bdd, gg, node):
    prios = []
    for curr_p_f in range(0, gg.k):
        for i in range(0, gg.d[curr_p_f] + 1):
            if bdd.let(node, gg.gamma[curr_p_f][i]) == bdd.true:
                prios.append(i)
                break
    return prios
