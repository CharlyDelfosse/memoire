...
def random_phi_and_p(bdd, game_infos):
    phi_0 = bdd.false
    phi_1 = bdd.false
    gamma = [defaultdict(lambda: bdd.false) for _ in range(game_infos.k)]

    all_nodes = []
    all_possiblities = bdd.pick_iter(bdd.true, game_infos.q_vars)
    for i in range(0, game_infos.n):
        current_node_dict = next(all_possiblities)
...
def random_game_gen(bdd, n, p, i, o):
    game_infos = GraphGame_gen(bdd, n, p)
